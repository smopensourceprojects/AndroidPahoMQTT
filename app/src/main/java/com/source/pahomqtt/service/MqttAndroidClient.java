...
public class MqttAndroidClient implements IMqttAsyncClient {
	...
	public boolean removeMessage(IMqttDeliveryToken token) throws MqttException {
		if(null != connectToken && null != connectToken.getClient()) {
			return connectToken.getClient().removeMessage(token);
		}
		return false;
	}
	...
	public void onBroadcastEvent(Intent intent) {
	...
	public int getInFlightMessageCount() {
		if(null != connectToken && null != connectToken.getClient()) {
			return connectToken.getClient().getInFlightMessageCount();
		}
		return 0;
	}
